    ...
    <section id="contact" className="scroll-mt-24 rounded-[2rem] bg-card px-6 py-14 sm:px-10 sm:py-20 lg:px-14" aria-labelledby="contact-title">
      <div className="grid gap-14 lg:grid-cols-[0.85fr_1.15fr] lg:gap-20">
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-6">
          <div><label htmlFor="contact-name" className="mb-2 block text-sm font-semibold">Họ tên</label><input id="contact-name" type="text" autoComplete="name" placeholder="Nguyễn Văn A" {...register('name')} aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? 'contact-name-error' : undefined} className="w-full border-b border-border bg-transparent px-0 py-3 text-base text-text outline-none transition placeholder:text-muted/60 focus:border-accent" />{errors.name && <p id="contact-name-error" className="mt-2 text-xs text-red-500">{errors.name.message}</p>}</div>
